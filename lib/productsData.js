...
export const products = [
  ...
  { name: 'Caffè d’orzo', category: 'caffetteria', price: 2.0, currency: 'EUR' },
  { name: 'Caffè d’orzo', category: 'caffetteria', price: 3.5, currency: 'EUR', note: 'Seconda voce presente nel listino fornito' },
  ...
  { name: 'Calice di vino (nel calice piccolo)', category: 'vino', price: 5.0, currency: 'EUR' },
...
export function getProductPriceInfo(query) {
  ...
  if (matches.length === 1) {
    ...
    return {
      found: true,
      message: `${match.name}: €${match.price.toFixed(2)}`,
      matches,
    };
  }
